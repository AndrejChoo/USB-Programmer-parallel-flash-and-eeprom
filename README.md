...
# 25.02.24
Переписал софт на Qt. Теперь есть возможность использовать на Linux. Скомпилировал под Linux x64, Linux aarch64, Windows x64. В Qt версии не реализованы функции вкладки "Test HW". Для запуска необходимо установить QSerialport, возможно, потребуются ещё какие-то недостающие пакеты. Проверить не могу, так как у меня на всех ОС установлен Qt-creator со всем необходимым. Возможно, особенно на Fedora, будет необходимо предоставит права доступа к /dev/ttyACM0 - пишем в терминале: sudo chmod a+rw /dev/ttyACM0. На Windows также работает Qt-версия, но мне больше нравится C# версия программы.
